type ConvertBigInt<T> =
  T extends bigint ? string :
  T extends Array<infer U> ? Array<ConvertBigInt<U>> :
  T extends object ? { [K in keyof T]: ConvertBigInt<T[K]> } :
  T;

export default function format<T>(obj: T): ConvertBigInt<T> {
  if (typeof obj === 'bigint')
    return obj.toString() as ConvertBigInt<T>;
